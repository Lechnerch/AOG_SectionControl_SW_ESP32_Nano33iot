...
void process_Request()
{
    ...
    for (byte n = 0; n < WiFi_Server.args(); n++) {
        ...
        if (WiFi_Server.argName(n) == "DataTransfVia") {
            Set.DataTransVia = byte(WiFi_Server.arg(n).toInt());
        }
        if (WiFi_Server.argName(n) == "SectNumEnd") {
            argVal = WiFi_Server.arg(n).toInt();
            if ((argVal >= 1) && (argVal <= 100)) { Set.SectNumEnd = byte(argVal); }
        }
...
// WiFi Client Access Data
    ...
    */
    ...
    strcat(HTML_String, "<tr><td><b>number of sections</b></td><td><input type = \"number\"  onchange=\"sendVal('/?SectNumEnd='+this.value)\" name = \"SectNumEnd\" min = \"1\" max = \"100\" step = \"1\" style= \"width:200px\" value = \"");
    strcati(HTML_String, Set.SectNumEnd);
    strcat(HTML_String, "\"></td>");
